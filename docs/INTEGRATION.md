...
| 绑定市场 | `0x000000000000000000000000000000000000000000000000000000000001a726` |
...
Stage 2（等待结算 → `syncMarketAndBonus` → 赎回 bonus）在窗口到期后由 **Somnia Reactivity
订阅自动触发**（handler `onEvent` → campaign `syncMarketAndBonus`），无需人工运行
`demo-endtoend.js 2`，也无需 off-chain keeper。
...
| `bonusBudget` | `0` | 市场作废，条件 bonus 正确地不触发 |
| `bonusTask.state` | `7`（Skipped） | 追加任务被跳过，资金不浪费 |
| `refundBonus` | `25000000`（25 tUSDC） | uniform void 退回一半 bonus（50 → 25） |
| Reactivity 结算 tx | `0x6555af95969a94b4c830f8dbcb736929204f8b73c63da6d09828412a14755cbe` | `onEvent` → `syncMarketAndBonus` |

结论：**「无条件 base + 有条件 bonus」双阶段协议在链上端到端成立**。基础任务无论市场
方向如何都有 100 tUSDC 预算；追加任务只在约定方向获胜时才释放（本次市场作废，正确跳过并
退回一半 bonus）。
